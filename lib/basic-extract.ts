...
interface PathAtom {
  type: 'atom',
  term: Term | ExtractedPath;
  zeroOrMore: boolean;      // Indicates whether '*' is applied or not
  zeroOrOne: boolean;       // Indicates whether '?' is applied or not
}

interface ExtractedPath {
  type: 'path',
  path: PathAtom             // A path sequence
  branches?: ExtractedPath[] // Paths to branch off at at the end of this path
  propertyFocus: boolean     // To denote whether this is the focus node of a particular property - if so extra data may be needed
}
...
async function extractPath(term: Term, engine: ExtendedEngine, propertyFocus = false): Promise<ExtractedPath> {
